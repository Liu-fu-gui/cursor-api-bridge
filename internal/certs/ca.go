...
// LoadOrCreateManager 为每台电脑生成独立 CA，私钥不会编译进应用或离开本机。
func LoadOrCreateManager(caCertPath, caKeyPath string) (*Manager, []byte, error) {
	if certPEM, keyPEM, err := loadCAPEMFromFiles(caCertPath, caKeyPath); err == nil {
	...
	template := &x509.Certificate{
		SerialNumber: serial,
		Subject: pkix.Name{
			CommonName:   "Cursor API 转换器 本地 CA",
			Organization: []string{"Liufugui Local Software"},
		},

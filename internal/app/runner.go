...
func Run(resources EmbeddedResources) error {
	...
	certManager, localCACertPEM, err := certs.LoadOrCreateManager(appdata.CACertFilePath(), appdata.CAKeyFilePath())
	if err != nil {
		return err
	}
	logEmbeddedCAInfo(localCACertPEM)
	...
	app := application.New(application.Options{
		...
		SingleInstance: &application.SingleInstanceOptions{
			UniqueID: "local.liufugui.cursor-agent-bridge.single-instance",
	...
	app.Event.OnApplicationEvent(events.Common.ApplicationStarted, func(event *application.ApplicationEvent) {
		...
		if os.Getenv("CURSOR_BRIDGE_NO_AUTOSTART") == "1" || strings.Contains(buildinfo.CurrentVersion(), "test") {
			logger.Infof("测试模式：跳过代理启动和 Cursor 配置修改")
			return
		}
		go func() {
